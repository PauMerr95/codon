...
```
> 00 00 00 00 = bits in a byte
> 00 11 22 33 = available loci
```

A codon is read left to right, and at first, the start is evaluated by a marker => 01 or 10, which signifies the start of nucleotides inside the codon. Conversely, any 00 or 11 before combinations before a marker are ignored .
```
> 01 00 00 00 = Marker at LOC0
  ^^
> 00 10 00 00 = Marker at LOC1
     ^^
> 11 11 01 00 = Marker at LOC2
        ^^
> 00 00 00 01 = Marker at LOC3 // invalid (would leave no space for any nucleotides)
           ^^
```
...
You might have noticed that there exists a certain redundancy in the last example and in the design of the markers. The reason for this is that the marker not only signals the start of the nucleotides; it also contains information about the orientation of this specific codon.

```
> 01 Marker = 5' => 3'
  (or N => C terminus for proteins)

> 10 Marker = 3' => 5'
  (or C => N terminus for proteins)
```
...
```
> 11 01 11 01 = 5'-TG-3'
  -- ^^ -- -- = Marker for 5' => 3' despite the 11 prefix
```
### bp location / shift
...
VOIDs have a few peculiarities in combination with more advanced features that you should watch out for:
* VOIDs contains 0 bases, so getting the length from the codon will return 0. In a sequence, however, this might be accounted for when calculating the number of stored objects. (More in future chapters. See [Seq_Advanced](docs/Seq_Advanced.md) on getters)
  ```c++
  codon::Codon empty_codon("VOID");  // create a void codon
  codon::Seq empty_seq(empty_codon); // create a void seq
  empty_codon.get_bases_len();  // 0
  empty_seq.get_seq_len();      // 1 🤯
  ```
* A VOID sequence (a Seq Object with only one or more VOIDs) behaves abnormally:
    * Returned locators from getters are invalid (More in future chapters. See [Seq](docs/Seq.md) on locators and [Seq_Advanced](docs/Seq_Advanced.md) on getters):
...
> ⚠️ Don't use VOID unless absolutely necessary.
...
```c++
codon::Codon start_codon("AGT"); // creates a codon object representing "5-AGT-3"
start_codon.get_bases_encoded(); // returns equivalent, printable ASCII character
```
[//]: # (TODO: Add more information about encoded characters)

> ⚠️ If you intend to store your data, prefer the encoded character format provided in the library so you can seamlessly import it again when needed.
