...
codon::Seq string_seq("&fe^h.d", codon::format::encoded);
//

// Will alway create a version that is full on the 5' end.
...
[!> [!NOTE]
An important feature of locators is that they are end-inclusive, meaning that unlike iterators when passed to a method they will always include the final location. There is no traditional .end() that points past the final object.
> ]
