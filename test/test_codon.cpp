...
void test::check_creation_str(std::vector<std::string> arr_bases, std::vector<codon::Codon> &generator) {
    int idx_gen = 0;
    for (std::string bases : arr_bases) {
        codon::Codon triplet_temp = codon::Codon(bases);
        assert(bases == triplet_temp.get_bases_str());
...
void test::check_operations(std::vector<codon::Codon> arr_codons) {
    ...
    for (codon::Codon temp_codon: arr_codons) {
         ...
         * and putting the dropouts into the final_codon, reimplementing the original order
         * (Previously inserted As stay in the reverse_codon)
         */
        while (original_len--){
...
int test::codon_test() {
    ...
    std::vector<std::string> arr_bases_str = {
        "TCA", "GGG", "AGC", "GTA", "CAT", "TTT", "ACT", "AAA", "VOID",
        "GG", "AA", "TC", "CA", "T", "A", "GGA", "ACG", "C", "CC",
        "AC", "CT", "GAC", "CGA", "CAG", "CAA", "SWITCH", "ACC", "TAA", "TTA"
    };
    codon::base arr_bases[4] = { codon::A, codon::G, codon::C, codon::T };

    std::vector<codon::Codon> codons_generated(arr_bases_str.size(), codon::Codon("VOID"));
